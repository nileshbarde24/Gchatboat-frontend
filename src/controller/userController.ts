    ...
    try {
        ...
        if (user) {
            try {
                ...
                const updateUserEmail = await db.collection('users').updateOne(
                    { _id: user._id },
...
// @ts-ignore
async function getCompanyByUserId(req: Request, res: Response) {
    try {
        const db = await getDatabase();
...
// async function updateCompanyByUserId(req: Request, res: Response) {
//     try {
//         const db = await getDatabase();
//         const adminUserId = req.params.id; // Get the user ID from the URL parameter
//         const userId = req.body.userId;
...
// @ts-ignore
async function sendOtpForForgotPassword(req: Request, res: Response) {
    try {
        const db = await getDatabase();
        const { email } = req.body;
        const isEmailExists = await db.collection('users').findOne({ email })
...
// @ts-ignore
async function matchOtpVerify(req: Request, res: Response) {
    try {
        const db = await getDatabase();
...
//get All Files
// @ts-ignore
async function getAllFilesWithUsers(req: Request, res: Response) {
    try {
        const db = await getDatabase();
