...
import {
    ...
    getFilesRelevantDocuments,
    addDocumentsToMemoryVectorStore,
    getGlobalHistory,
    getFilesHistory,
    getVectorStore,
    storeFileToDB
} from "../lib/dbVectorStore.js";
...
async function selectedFilesStartConversation(req: Request, res: Response) {
    const uniqueId = uuidv4();
...
// local image
async function analyzeImageLocalOrUrl(req: Request, res: Response) {
    ...
    if (req.files) {
        ...
        // @ts-ignore
        files = req.files.map((file: any) => encodeImage(file.path))
        formattedFiles = files.map((fileBase64: any) => ({
            type: "image_url",
            image_url: {
...
// Web Scraping:
async function webScrap(req: Request, res: Response) {
    try {
        // @ts-ignore
        const userId = req?.userId;
        ...
        const flattenedDocuments = formattedDocs.reduce((acc, val: any) => acc.concat(val), []);
...
async function generateRelevantSentence(req: Request, res: Response) {
    ...
    try {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "user",
                    content: `Generate a short and relevant summary for the following input to be used as a chat history title: "${userInput}"`,
                },
            ],
            max_tokens: 60,
        });
        ...
        if (response.choices && response.choices.length > 0 && response.choices[0].message) {
            let generatedSentence = response.choices[0].message.content?.trim();
            if (generatedSentence) {
                generatedSentence = generatedSentence.replace(/^"|"$/g, '');
                console.log('Generated Sentence:', generatedSentence);
                return res.status(200).json({ data: generatedSentence, message: 'Sentence fetched successfully' });
            } else {
                console.error("Message content is empty.");
                return res.status(400).json({ message: 'Unable to fetch sentence' });
            }
        } else {
            console.error("No choices found in the response.");
            return res.status(401).json({ message: 'Error creating sentence' });
        }
        // const generatedSentence = response.choices[0]['message']['content'].trim();
...
export {
    startConversation,
    selectedFilesStartConversation,
    analyzeImage,
    analyzeImageLocalOrUrl,
    webScrap,
    generateRelevantSentence
}