...
import {
  ...
  getProjectRoot,
  // setCurrentVectorStoreDatabasePath 
...
// @ts-ignore
async function storeData(documents: any) {

  let combinedText: any[] = [];

  function combineArrayOfSets(arrayOfSets: any) {
    if (arrayOfSets && arrayOfSets.length > 0) {
      arrayOfSets?.forEach((sets: any) => {
        combinedText.push(sets?.pageContent);
      });
    }
  }

  combineArrayOfSets(documents);

  // Join all pageContent with line breaks
  const finalText: any = combinedText.join('\n');
  const outputPath: string = path.join(projectRootDir, 'output', 'output.txt');
  // Write the combined text to the output file
  fs.writeFile(outputPath, finalText, { flag: 'w' }, (err) => {
    if (err) {
...
async function loadAndSplitFile(filePath: string): Promise<Document<Record<string, unknown>>[]> {
  ...
  const options = {
    ...
    // apiKey: "RcPvTxoGPgW2paIeb32MDIyzsUlzeS", // old
    apiKey: "ZJcaGeJYbfjTUSlZaN8QYceT5X31js", // new - 1000 pages / month
    strategy: "hi_res",
  };
  ...
  switch (fileExtension) {
    ...
    case '.pdf':
      ...
      if (documents.length === 0) {
        loader = new UnstructuredLoader(filePath, options);
        documents = await loader.loadAndSplit(new RecursiveCharacterTextSplitter());
      }
    ...
    case '.csv':
      ...
      documents = await loader.loadAndSplit(new RecursiveCharacterTextSplitter({chunkOverlap:0}));
      await storeData(documents)
      break;
    case '.epub':
...
//     // console.log("OpenAIEmbeddings Response: ", vectorStore)
//   } catch {
//     spinner = ora({
//       ...defaultOraOptions,
