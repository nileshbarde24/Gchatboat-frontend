...
async function getRelevantDocuments(userId: string, question: string) {
...
// Using MMR in a vector store retrieve
async function getFilesRelevantDocuments(userId: string, uploadedFileIdz: any, question: string) {
    const contextVectorStore = await getVectorStore();
...
async function getGlobalHistory(userId: string, topicId: string, question: string) {
...
async function getFilesHistory(userId: string, topicId: string, question: string, filesIdzStrArr: any) {
    const historyVectorStore = await getMemoryVectorStore();
...
async function addDocumentsToMemoryVectorStore(documents: Array<{
    content: string;
    metadataType: string,
    qaPairID: string,
    userId: string,
...
// @ts-ignore
async function storeFileToDB(filename: string, userId: string, isUrl: boolean) {
    const timestamp = new Date().toISOString();
    let serverURL
