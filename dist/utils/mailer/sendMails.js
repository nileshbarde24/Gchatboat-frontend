...
async function addUserVerifyMail(email, token, firstName, lastName, password) {
    const mailData = AddUserVerifyEmail(SENDER_EMAIL, email, token, firstName, lastName, password);
    await sendEmail(mailData);
}
;
// @ts-ignore
async function otpForgotPasswordMail(email, otp) {
    const mailData = ForgotPasswordOTP(SENDER_EMAIL, email, otp);
    sendEmail(mailData);
}
;
// @ts-ignore
async function otpForSetupEmailAuthenticationMail(email, otp) {
    const mailData = SetupMultiFactOfEmailOtp(SENDER_EMAIL, email, otp);
    sendEmail(mailData);
}
;
// @ts-ignore
