...
// @ts-ignore
async function deleteDataOfSingleFile(req, res) {
    try {
        ...
        if (deleteTopicResult.deletedCount > 0 ||
...
// @ts-ignore
async function deleteDataOfMultipleFile(req, res) {
    try {
        const db = getDatabase();
...
;
// utils:
async function removeFileFromUploadsDirectory(fileNameToRemove) {
...
async function getFileById(uploadedFileId) {
    const db = getDatabase();
    let fileRemoveResp;
...
async function getAllFilesById(uploadedFileIds) {
    const db = getDatabase();
    // @ts-ignore
...
//# sourceMappingURL=filesActionController.js.map